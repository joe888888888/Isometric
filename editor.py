...
class Grid:
    ...
    def transform_locations(self):
        ...
        shift_y = pg.display.get_window_size()[1] / 1.5
        ...
        if edit_mode_flag:
            ...
            for i in range(self.num_nodes):
                d = math.dist(mouse_pos, (self.iso_grid[self.current_slice][i]["coords"][0] + self.scale_factor,
                                          self.iso_grid[self.current_slice][i]["coords"][1]))
    ...
    def draw_locations(self):
        ...
        # draw blocks
        for i in range(self.num_nodes):
            if self.sorted_iso_grid[self.current_slice][i]["occupancy"] != 0:
